...
import { Item, DeliveryOrder } from '@/types/api';
...
export default function EditDeliveryOrder() {
  ...
  const loadOrder = async () => {
    try {
      const order = await DeliveryOrdersService.getOrderById(parseInt(id!));
      const cartItems = order.deliveryItems.map((di) => ({
        item: di.item,
        quantity: di.quantity,
      }));
      setCart(cartItems);
      // Customer info would come from API if available
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    try {
      await DeliveryOrdersService.updateOrder(parseInt(id!), {
        status: 'DRAFT',
  ...
  return (
    <Layout>
      <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6 pb-20 md:pb-6">
        <h1 className="text-xl font-semibold">Edit Delivery Order - DO{id}</h1>
